 ...
 * 天气对比的**对外契约**。
 *
 * 这些类型前后端共用：后端按它归一化两家数据源，前端照它渲染。
 * 从 weather-app 搬迁时，前端那份 `types/weather.ts` 是这份的**手抄副本** ——
 * 它的 CLAUDE.md 里写着「改后端契约必须同步改前端类型，没有任何自动检查会提醒你」。
 * 挪到这里之后，改一个字段名两边会同时编译失败。那就是那个自动检查。
 *
 * 服务端内部接口（WeatherQuery、WeatherProvider）**不在这里** ——
 * 它们进来会让前端看见本不该看见的边界，时间一长这个包就退化成公共垃圾桶。
 */
