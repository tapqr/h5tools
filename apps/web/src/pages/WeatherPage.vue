...
import { requestCurrentLocation } from '../weather/composables/useGeolocation';
import CitySearch from '../weather/components/CitySearch.vue';
import ProviderCard from '../weather/components/ProviderCard.vue';
import HourlyTrendChart from '../weather/components/HourlyTrendChart.vue';
import WeatherIcon from '../weather/components/WeatherIcon.vue';
import type { NormalizedLocation } from '@h5tools/shared';
import { fetchReverseLocation } from '../weather/api/geo';
import { formatLocationName } from '../weather/utils/location-display';
import { classifyCondition, formatTemperature, resolveDayPart, type DayPart } from '../weather/utils/weather-display';
import { alignHourly, type OkProvider } from '../weather/utils/align';
 ...
 * 昼夜基调。
 *
 * ⚠ 搬迁时把它从 `document.documentElement` 挪到了页面根节点：
 * 原项目是独立应用，写在 <html> 上没问题；搬进多页应用后那是全局污染 ——
 * 离开天气页后 data-daypart 还留着，配色变量会跟着泄漏到别的页面。
 * weather.css 的选择器同步降成了 `.weather-page[data-daypart]`。
 */
function applyDayPart() {
  const part = resolveDayPart(new Date());
  daypart.value = part;
  // 让移动端浏览器的地址栏/状态栏跟着一起变,这是"像原生应用"很关键的一环
  themeColorMeta()?.setAttribute('content', THEME_COLORS[part]);
}
...
// 进页面时记下原值，离开时还原 —— 否则 theme-color 会一直停在天气页的颜色上
...
<template>
  <!--
    两层结构，对应搬迁前的 body + #app：
    外层 .weather-page 铺满整屏放渐变背景，内层 .app 限宽 520px 居中。
    合成一层的话，要么背景填不满屏，要么桌面上内容被拉成整屏宽。
  -->
  <div class="weather-page" :data-daypart="daypart">
    <main class="app">
...
.app {
  min-height: 100svh;
  /* 桌面上不铺满整屏 —— 这是个 H5 页面，按手机宽度居中更接近它真实的使用形态。
     搬迁前这条在全局的 #app 上，现在收进页面自己的 scoped style。 */
  max-width: 520px;
  margin: 0 auto;
